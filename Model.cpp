...
Model::Model(std::string path) {
	...
	const aiScene* scene = importer.ReadFile(path, aiProcess_Triangulate | aiProcess_ConvertToLeftHanded | aiProcess_GenNormals);
...
void Model::LoadBones(const aiMesh* mesh, std::vector<Model::BoneData>& boneData) {
...
void Model::ReadNodeHierarchy(const aiNode* node, int parentIndex) {
	BoneInfo boneInfo;
...
void Model::LoadTexturesFromFile(ID3D12Device* device, ID3D12GraphicsCommandList* cmdList) {
